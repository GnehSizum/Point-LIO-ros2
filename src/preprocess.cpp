...
void Preprocess::mid360_handler(const std::shared_ptr<const sensor_msgs::msg::PointCloud2> &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
...
void Preprocess::default_handler(const std::shared_ptr<const sensor_msgs::msg::PointCloud2> &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
...
void Preprocess::pub_func(PointCloudXYZI &pl, const rclcpp::Time &ct)
{
  ...
  output.header.stamp.nanosec = ct.nanoseconds() % 1000000000ull;
}
