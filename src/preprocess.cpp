...
void Preprocess::process(const std::shared_ptr<const livox_ros_driver2::msg::CustomMsg> &msg, PointCloudXYZI::Ptr &pcl_out)
{  
  avia_handler(msg);
  *pcl_out = pl_surf;
}
...
void Preprocess::process_cut_frame_livox(const std::shared_ptr<const livox_ros_driver2::msg::CustomMsg> &msg, 
                                        deque<PointCloudXYZI::Ptr> &pcl_out, deque<double> &time_lidar, const int required_frame_num, 
                                        int scan_count) {
...
void
Preprocess::process_cut_frame_pcl2(const std::shared_ptr<const sensor_msgs::msg::PointCloud2> &msg, deque<PointCloudXYZI::Ptr> &pcl_out,
...
void Preprocess::avia_handler(const std::shared_ptr<const livox_ros_driver2::msg::CustomMsg> &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
...
void Preprocess::oust64_handler(const std::shared_ptr<const sensor_msgs::msg::PointCloud2> &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
...
void Preprocess::velodyne_handler(const std::shared_ptr<const sensor_msgs::msg::PointCloud2> &msg)
{
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();
...
void Preprocess::hesai_handler(const std::shared_ptr<const sensor_msgs::msg::PointCloud2> &msg)
{
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();
